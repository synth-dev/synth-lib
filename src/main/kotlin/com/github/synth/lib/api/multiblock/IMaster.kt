...
interface IMaster<T : BlockEntity> : ITile<T> {
    ...
    private fun locateMinimum(pos: BlockPos, forward: Direction): Opt<BlockPos> {
        ...
        if (!isBlockValidVariant(block, VerticalEdge, Opt.nil())) {
            ...
            if (!isBlockValidVariant(block, VerticalEdge, Opt.nil())) {
                error {
                    "We iterated to an invalid block! expected a vertical edge but found ${
                        world().getBlockState(
                            block
                        ).block.name.string
                    }, at [${block.toShortString()}]"
                }
                return Opt.nil()
            }
        }
    ...
    fun isValid(structureIn: Opt<StructureStore>): Boolean {
        ...
        for ((block, _) in structure[VerticalEdge]) if (!isValidVerticalEdgeVariant(
                block,
                level,
                structureIn
            )
        ) return false
        for ((block, _) in structure[HorizontalEdge]) if (!isValidHorizontalEdgeVariant(
                block,
                level,
                structureIn
            )
     ...
     */
    fun isValidSideVariant(blockPos: BlockPos, level: Level, structure: Opt<StructureStore>): Boolean =
        isBlockValidVariant(blockPos, Side, structure)

    /**
     * return true when the inner variant is valid, can add custom specifications here relative to inner validation
     */
    fun isValidInnerVariant(blockPos: BlockPos, level: Level, structure: Opt<StructureStore>): Boolean =
        isBlockValidVariant(blockPos, Inner, structure)
