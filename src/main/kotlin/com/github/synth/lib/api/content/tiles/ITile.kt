package com.github.synth.lib.api.content.tiles

import com.github.synth.lib.api.util.*
import com.github.synth.lib.api.util.Facing.*
...
interface ITile<T : BlockEntity> {
     ...
     */
    fun relativeDir(side: Facing): Direction {
        val forward = forwardDir()
        return when (side) {
            Top -> if (forward == UP || forward == DOWN) NORTH else UP
            Facing.Bottom -> if (forward == UP || forward == DOWN) NORTH.opposite else DOWN
     ...
     */
    fun <T> neighborCap(side: Facing, capability: Capability<T>): LazyOptional<T> {
